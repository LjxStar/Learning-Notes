...
```
- `<head>`：网页头部，存放浏览器解析所需的元信息（CSS 样式、页面标题、编码等），不对用户可见
...
```
- `target="_blank"` 新窗口打开；`_self` 当前页打开（默认）

#### 3.容器与列表标签
- `<div>`：块级容器，独占一行，用于页面布局，可设置宽高
- `<span>`：行内容器，一行可放置多个，用于局部文字修饰，不可设置宽高
...
3. **外部样式**：独立 `.css` 文件，通过 `<link>` 引入，多页面复用，开发推荐
